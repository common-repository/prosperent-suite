...
abstract class Model_Base
{
	...
	private $_subinterfaces = array(
		'insert' => '&interface=wp&subinterface=prosperinsert',
		'shop'   => '&interface=wp&subinterface=prospershop',
		'links'  => '&interface=wp&subinterface=prosperinsert'
	);
    ...
    {
        wp_enqueue_style( 'prosper-font-awesome', '//maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css', array(), '4.4');
    }
	...
	{
		$adminUrl = home_url('', (is_ssl() ? 'https' : 'http')) . $_SERVER["REQUEST_URI"];

		if ($_POST['prosperAccess'])
        {
        	$genOpts = get_option('prosperSuite');
        	$genOpts['Api_Key'] = $_POST['Api_Key'];
       	...
       	echo '<form action="' . $homeUrl . $_SERVER["REQUEST_URI"] . '" method="post" id="prosper-conf"><input type="hidden" id="hidden_prosperAccess" name="prosperAccess"><input type="hidden" id="hidden_Api_Key" name="Api_Key"></form>';
		echo '<div class="error" style="padding:6px 0;">';
		echo _e( '<span style="font-size:14px; padding-left:10px;">You have not connected to Prosperent.</span></br>', 'my-text-domain' );

		echo _e('<span style="font-size:14px; padding-left:10px;">Please <a onClick="prosperConnect(this);return false;" href="https://prosperent.com/account/index/index/message/Sign%20in%20to%20connect%20the%20WordPress%20Plugin/connect/1/redir/' . urlencode($adminUrl . '?&connected=true') . '" target="_self">connect</a> to Prosperent to link your account.</span></br>', 'my-text-domain' );
		echo _e('<span style="font-size:14px; padding-left:10px;">The Prosperent Suite will not work till you are connected.</span>', 'my-text-domain' );
		echo '</div>';
	}
	...
	{
		$optarr = array('prosperSuite', 'prosper_productSearch', 'prosper_autoComparer', 'prosper_prosperLinks', 'prosper_advanced');
	...
	public function prosperRewrite()
	{
		...
		add_rewrite_rule('^([^/]+)/([^/]+).cid.([a-z0-9A-Z]{32})/?$', 'index.php?pagename=' . $pageName . '&prosperPage=$matches[1]&keyword=$matches[2]&cid=$matches[3]', 'top');
		add_rewrite_rule($pageName . '/' . '(.+)', 'index.php?pagename=' . $pageName . '&queryParams=$matches[1]', 'top');
	}
	...
	public function prosperStoreInstall()
	{
	    ...
	    {
       		// the menu entry...
    		add_option("prosperent_store_page_title", 'Products', '', 'yes');
    		// the slug...
	...
	public function prosperStoreRemove()
	{
	    ...
	    if($pageId)
	    {
	        wp_trash_post($pageId); // this will trash, not delete
	    }
	}
	...
	{
	    if (isset($this->_options['prosperSid']))
	    {
	        $sidArray = array();
	...
	public function apiCall ($settings, $fetch, $sid = '')
	{
		...
		$settings = array_merge(array(
			...
			'location'  	  	 => $homeUrl . filter_var($_SERVER['REQUEST_URI'], FILTER_SANITIZE_URL),
	...
	public function multiCurlCall ($urls = array(), $expiration = 86400, $settings = array())
	{
		...
		if ($result === FALSE)
		{
			...
			{
				//$response[$id] = wp_remote_get( $url, $args );
				$result[$id] = json_decode(wp_remote_retrieve_body( wp_remote_get($url, $args ) ),true);
				//$result[$id] = json_decode( $response[$id]['body'] , true);
			}
*/
