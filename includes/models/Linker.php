...
class Model_Linker extends Model_Base
{
	...
	public function createLinkShortCode($atts, $content = null)
	{
		...
		$domain = stripos('http', strtolower($pieces['sl'])) ? $pieces['sl'] : 'http://' . $pieces['sl'];

		$affUrl = 'http://prosperent.com/wp/linkaffiliator/redirect?apiKey=' . $options['Api_Key'] . '&location=' . rawurlencode($currentUrl) . '&sid=' . $sid . '&url=' . rawurlencode($domain);
		$rel = 'nofollow,nolink';

		return '<a class="prosperent-kw shopCheck" style="text-decoration:none;" href="' . $affUrl . '" TARGET=' . $target . '" rel="' . $rel . '">' . $content . '</a>';
	}
	...
	public function linkerShortcode($atts, $content = null)
	{
		...
		$merchants = $pieces['mid'] ? str_replace(',', '|', $pieces['mid']) : '';
...
// 		}
		...
		if ($pieces['ft'] == 'fetchProducts')
		{
			...
			$settings = array(
				...
				'filterMerchantId' => (!$pieces['id'] ? $merchantIds : ''),
				'filterBrand'	   => (!$pieces['id'] ? $brands : ''),
				'filterKeyword'    => $pieces['id'] ? str_replace('_', ' ', $pieces['id']) : '',
		...
		elseif ($pieces['ft'] == 'fetchMerchant')
		{
			...
			$settings = array(
				...
				'filterMerchantId' => $pieces['id'] ? $pieces['id'] : '',
		...
		if ($pieces['ft'] == 'fetchMerchant')
		{
			if ($allData['data'][0]['deepLinking'] == 1)
			{
				...
				$affUrl = 'http://prosperent.com/wp/linkaffiliator/redirect?apiKey=' . $options['Api_Key'] . '&location=' . rawurlencode($currentUrl) . '&sid=' . $sid . '&url=' . rawurlencode('http://' . $allData['data'][0]['domain']);
				$rel = 'nofollow,nolink';
			}
			else
			{
	...
	public function autoLinker($text)
	{
		...
		if (!empty($options['Match'][0]))
		{
			...
			$page 	  		  = get_post($pageId);
		    $random 		  = FALSE;
		    $basePage         = $page->post_name;
		    $base   		  = $basePage . '/query/';
		    $target 		  = '_self';
		    $productSearchUrl = (is_ssl() ? home_url('/', 'https') : home_url('/', 'http')) . $base;
            $fetch            = 'fetchProducts';
