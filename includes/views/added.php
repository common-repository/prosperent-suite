...
if ($params[$type.'images'])
{
	...
	echo '<div style="font-weight:bold;">Selected ' . ($type == 'prod' ? 'Product' : 'Merchant') . '(s) - ' . count($images) . '</div>';
	echo '<div id="prosperAdded' . $params['type'] . '" style="display:block;overflow:auto;border:1px solid #919B9C;background-color:#fff;padding:2px 0;' . ($type == 'prod' ? 'height:84px;' : 'height:56px;') . '">';
    	echo '<div id="stickyHeader">';
        	foreach ($images as $i => $image)
        	{
        	    echo '<img id="small' . htmlspecialchars($id[$i]) . '" style="width:' . $width . ';padding:0 2px;" class="smallImage" src="' . $image . '"  alt="' . $keywords[$i] . '" title="' . $keywords[$i] . '" onclick="getIdofItem(this, true)"/>';
        	}
    	echo '</div>';
    echo '</div>';
}
