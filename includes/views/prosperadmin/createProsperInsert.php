...
var t;function getNewCurrent(){return"prod"}
...
function setFocus(){document.getElementById("prodquery").focus()}
...
function getFilters(){jQuery("#prodquery").val()!=window.queryString&&""!=window.queryString&&(jQuery("#prodd").val(""),jQuery("#prodb").val(""),showValues());var c=jQuery("#prodd").val()?jQuery("#prodd").val().replace(",","|"):"",d=jQuery("#prodb").val()?jQuery("#prodb").val().replace(",","|"):"",b=(jQuery("#pricerangea").val()?jQuery("#pricerangea").val()+",":"0.01,")+(jQuery("#pricerangeb").val()?jQuery("#pricerangeb").val():""),e=jQuery("#onSale:checked").val()?"1,":(jQuery("#percentrangea").val()?
jQuery("#percentrangea").val()+",":"")+(jQuery("#percentrangeb").val()?jQuery("#percentrangeb").val():"");if(c||d||e||jQuery("#prodquery").val())jQuery.ajax({type:"POST",url:"http://api.prosperent.com/api/search",data:{api_key:prosperSuiteVars.apiKey,query:jQuery("#prodquery").val(),filterBrand:d,filterPrice:b,filterPercentOff:e,limit:1,enableFacets:"merchantId|merchant",enableFullData:0},contentType:"application/json; charset=utf-8",dataType:"jsonp",success:function(b){jQuery("#prodmerchant").empty();
jQuery.each(b.facets.merchantId,function(d,e){c.match(e.value)?jQuery("#prodmerchant").append('<li id="d'+e.value+'" class="activeFilter" onClick="getIdValue(this);getFilters();"><a href="javascript:void(0);"><span>'+b.facets.merchant[d].value+"</span></a></li>"):jQuery("#prodmerchant").append('<li id="d'+e.value+'" onClick="getIdValue(this);getFilters();"><a href="javascript:void(0);"><span>'+b.facets.merchant[d].value+"</span></a></li>")})},error:function(){}}),jQuery.ajax({type:"POST",url:"http://api.prosperent.com/api/search",
data:{api_key:prosperSuiteVars.apiKey,query:jQuery("#prodquery").val(),filterMerchantId:c,filterBrand:d,filterPrice:b,filterPercentOff:e,limit:1,enableFacets:"brand",enableFullData:0},contentType:"application/json; charset=utf-8",dataType:"jsonp",success:function(b){jQuery("#prodbrand").empty();jQuery.each(b.facets.brand,function(b,c){d.match(c.value)?jQuery("#prodbrand").append('<li id="b'+c.value+'" class="activeFilter" onClick="getIdValue(this);getFilters();"><a href="javascript:void(0);"><span>'+
c.value+"</span></a></li>"):jQuery("#prodbrand").append('<li id="b'+c.value+'" onClick="getIdValue(this);getFilters();"><a href="javascript:void(0);"><span>'+c.value+"</span></a></li>")})},error:function(){}}),window.queryString=jQuery("#prodquery").val()}
...
function showAddedValues(c){c="";c=jQuery("form").serialize();xmlhttp=new XMLHttpRequest;xmlhttp.onreadystatechange=function(){jQuery("div.prodadded").html(xmlhttp.responseText).show()};var d=window.location.pathname,d=d.substring(0,d.lastIndexOf("wp-admin/"))+"wp-content/plugins/prosperent-suite/includes/views/added.php?type=prod&";xmlhttp.open("GET",d+c,!0);xmlhttp.send()}
function sticky_relocate(){var c=jQuery(window).scrollTop(),d=jQuery("#sticky-anchor").offset().top;c>d?jQuery("#stickyHeader").addClass("sticky"):jQuery("#stickyHeader").removeClass("sticky")}
jQuery(function(){window.queryString="";window.prodqueryString="";window.merchantqueryString="";window.imgLoc=prosperSuiteVars.imageLoc;document.getElementById("apiKey").value=prosperSuiteVars.apiKey;jQuery(window).keydown(function(c){if(13==c.keyCode)return c.preventDefault(),!1});jQuery(window).scroll(sticky_relocate);sticky_relocate();jQuery("#prosperMCE_submit").on("click",function(c){c.preventDefault();c=jQuery("#prodfetch").val();var d=jQuery("#prodquery").val(),b=jQuery("#prodd").val(),e=jQuery("#prodlimit").val(),
...
C=jQuery("#noShow:checked").val(),D=jQuery("#prodImageType").val(),E=jQuery("#sBarWidth").val(),F=jQuery("#widthStyle:checked").val(),G=jQuery("#percentrangea").val(),H=jQuery("#percentrangeb").val(),I=jQuery("#prodcategory").val(),g=jQuery("#prosperHeldURL").val(),J=window.prodqueryString,a="["+v;d&&(a+=' q="'+d+'"');h&&(a+=' gtm="'+h+'"');k&&(a+=' b="'+k+'"');b&&(a+=' mid="'+b+'"');e&&(a+=' l="'+e+'"');f&&(a+=' v="'+f+'"');m&&(a+=' id="'+m.replace(/notfound~/g,"")+'"');c&&(a+=' ft="'+c+'"');w&&
(a+=' css="'+w+'"');n&&(a+=' q="'+n+'"');p&&(a+=' utg="'+p+'"');q&&(a+=' utt="'+q+'"');r&&(a+=' sale="'+r+'"');x&&(a+=' sf="'+x+'"');y&&(a+=' sbar="'+y+'"');z&&(a+=' sbu="'+z+'"');l&&(a+=' vst="'+l+'"');C&&(a+=' noShow="'+C+'"');D&&(a+=' imgt="'+D+'"');I&&(a+=' cat="'+I+'"');J&&(a+=' fb="'+J+'"');E&&(a+=' w="'+E+'"');F&&(a+=' ws="'+F+'"');g&&"http://"!=g&&(a+=' ahl="'+g+'"');(A||B)&&(a+=' pr="'+A+","+B+'"');(G||H)&&(a+=' po="'+G+","+H+'"');u&&(a+=' gimgsz="'+u+'"');a+="][/"+v+"]";a.replace(/"/g,"&quot;");
jQuery(document.getElementById("shortCodeVal")).val(a);jQuery(document.getElementById("shortCodeBox")).show();jQuery(".wrap").get(0).scrollIntoView(!0)})});
function openImageType(){"pc"==jQuery("#prodview:checked").val()?(jQuery("#prodimages").val(""),jQuery("#prodid").val(""),jQuery(document.getElementById("prosperInsertLimit")).hide(),jQuery("#prodImageType").css("visibility","visible"),showAddedValues()):(jQuery(document.getElementById("prosperInsertLimit")).show(),jQuery("#prodImageType").css("visibility","hidden"))};
</script><?php
		...
		<table>
			<tr>
                ...
                <td style="vertical-align:middle;">
                    ...
                    <select tabindex="2" id="prodImageType" name="prodImageType" style="font-size:14px;visibility:hidden;width:120px;margin-left:50px!important;margin-bottom:4px!important;">
				    ...
				    <div style="float:right;z-index:1001;">
            			...
            			<input tabindex="11" type="submit" value="Create Short Code" class="button-primary" id="prosperMCE_submit"/>
                    </div>
                </td>
            </tr>
		</table>
