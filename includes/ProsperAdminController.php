...
class ProsperAdminController
{
    ...
    public function __construct()
    {
		...
		add_action('admin_init', array($prosperAdmin, 'prosperActivateRedirect'));
		add_action('admin_init', array($prosperAdmin, 'prosperCustomAdd'));
	...
	public function grantAccess()
	{
		...
		if ( !is_array( $options ) || !isset( $options['MultiSite_User'] ) )
			return true;

		if ( $options['MultiSite_User'] == 'superadmin' && !is_super_admin() )
			return false;
	...
	{
		add_menu_page( __('Prosperent Suite Settings', 'prosperent-suite'), __( 'Prosperent', 'prosperent-suite' ), 'delete_users', 'prosper_multisite', array( $this, 'networkConfigPage' ), PROSPER_IMG . '/prosperentWhite.png' );
	}
